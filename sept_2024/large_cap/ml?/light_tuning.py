...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep  8 09:46:09 2024
...
models = [
    ...
    LogisticRegression(max_iter=1000, C=0.5, penalty='l2'),
    LogisticRegression(max_iter=1000, C=1.0, penalty='l1', solver='saga'),
    
    DecisionTreeClassifier(),
    DecisionTreeClassifier(max_depth=10, min_samples_split=10, min_samples_leaf=5),
    DecisionTreeClassifier(max_depth=8, min_samples_split=20, min_samples_leaf=10),
    
    RandomForestClassifier(),
    RandomForestClassifier(n_estimators=200, max_depth=10, min_samples_split=10),
    RandomForestClassifier(n_estimators=150, max_depth=12, min_samples_split=5),
    GradientBoostingClassifier(),
    GradientBoostingClassifier(n_estimators=200, learning_rate=0.05, max_depth=5),
    GradientBoostingClassifier(n_estimators=150, learning_rate=0.1, max_depth=3),
    ...
    SVC(probability=True, C=1.0, gamma=0.1),
    SVC(probability=True, C=0.5, gamma=0.01),
    
    GaussianNB(),
    GaussianNB(priors=None),  # Default, let the model infer class priors
    GaussianNB(priors=[0.75, 0.25]),  # Manually set the priors based on class distribution
    
    MLPClassifier(max_iter=1000),
    MLPClassifier(max_iter=1000, hidden_layer_sizes=(100,), learning_rate_init=0.001, activation='relu'),
    MLPClassifier(max_iter=1000, hidden_layer_sizes=(50, 50), learning_rate_init=0.01, activation='tanh')
]
...
performances.sort(key=lambda x: (x[1], x[2], x[3], x[4]), reverse=True)
