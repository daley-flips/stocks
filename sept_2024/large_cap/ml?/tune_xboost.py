...
lrs = [0.01, 0.1, 0.25, 0.5]
depths = [2, 4, 6, 8]
bs = [0.0001, 0.001, 0.01, 0.1, 1, 10, 100]

total = len(trees)*len(lrs)*len(depths)*len(bs)
curr = 1
...
for tree in trees:
    for lr in lrs:
        for depth in depths:
            for b in bs:
                print(f'running {curr}/{total} runs')
                curr += 1
# *****************************************************************************
...
# *****************************************************************************
                    ...
                    scale_pos_weight = (neg_class / pos_class)*b
                    
                    # Create and train the model with `scale_pos_weight`
                    model = XGBClassifier(n_estimators=tree, learning_rate=lr, max_depth=depth, scale_pos_weight=scale_pos_weight)
...
# *****************************************************************************
                ...
                params = {'Trees': tree, 'lr': lr, 'depth': depth, 'b': b}
                performances.append((params, round(accuracy, 4), round(auroc, 4), tps, tns, fps, fns))
